def call(globalJsonSettings = '{}') {
  ...
  foundFiles.each {
    ...
    // Prisma Cloud CVE scan logic
    def prismaCloudConfig = readJSON file: 'prisma_cloud_config.json'
    withCredentials([usernamePassword(credentialsId: prismaCloudConfig.credentialsId, passwordVariable: 'prismaPassword', usernameVariable: 'prismaUsername')]) {
      try {
        def prismaScanResult = sh(returnStdout: true, script: "prisma-cloud-cli scan --image ${env.IMAGE_NAME}:${env.IMAGE_TAG} --user ${env.prismaUsername} --password ${env.prismaPassword}")
        if (prismaScanResult.contains("Vulnerabilities found")) {
        ...
        echo "Error occurred during Prisma Cloud scan."
        currentBuild.result = 'FAILURE'
        error("Error occurred during Prisma Cloud scan.")
      }
    }
